...
void process_lambda_interval(const std::vector<std::set<int>>& tumorData, 
                             ...
                             long long int &count, 
                             int rank, 
                             std::vector<std::array<int, 3>>& bestCombinations) 
{
    long long int total_count = 0;
    std::vector<std::array<int, 3>> total_bestCombinations;

    #pragma omp parallel
    {
        std::vector<std::array<int, 3>> local_bestCombinations;
            ...
            if (!intersectTumor1.empty()) {
                for (int k = j + 1; k < totalGenes; k++) {
                    ...
                    if (!intersectTumor2.empty()) {
                        ...
                        local_bestCombinations.push_back({i, j, k});
                        local_count++;
                    }
                }
            }
        ...
        #pragma omp critical
        {
            total_bestCombinations.insert(total_bestCombinations.end(),
                                          local_bestCombinations.begin(),
                                          local_bestCombinations.end());
            total_count += local_count;
        }
    }

    // Outside of parallel region, merge all results into global containers
    bestCombinations.insert(bestCombinations.end(),
                            total_bestCombinations.begin(),
                            total_bestCombinations.end());
    count += total_count;
}
...
int* worker_process(int rank, int num_workers, long long int num_Comb,
	...
	std::cout << "Total time spent in process_lambda_interval: " 
                  << total_process_lambda_time << " seconds." << std::endl;
	fflush(stdout);
