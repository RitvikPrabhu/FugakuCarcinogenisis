...
process_lambda_interval(LAMBDA_TYPE startComb, LAMBDA_TYPE endComb,
                        ...
                        SET &scratchBufferijklmn, double elapsed_times[]) {
  double alpha = 0.1;
      ...
      for (int l = k + 1; l < totalGenes - (NUMHITS - 4); l++) {
        ...
        for (int m = l + 1; m < totalGenes - (NUMHITS - 5); m++) {
          ...
          for (int n = m + 1; n < totalGenes - (NUMHITS - 6); n++) {
            ...
            SET_INTERSECT(scratchBufferijklmn, scratchBufferijklm, rowN,
                          dataTable.tumorRowUnits);
            END_TIMING(proc_intersect_ijklmn,
                       elapsed_times[PROCESS_LAMBDA_INTERSECT]);
            if (SET_IS_EMPTY(scratchBufferijklmn, dataTable.tumorRowUnits)) {
              continue;
            }

            for (int o = n + 1; o < totalGenes - (NUMHITS - 7); o++) {
              START_TIMING(proc_row_o);
              SET rowO =
                  GET_ROW(dataTable.tumorData, o, dataTable.tumorRowUnits);
              END_TIMING(proc_row_o, elapsed_times[PROCESS_LAMBDA_SET_COUNT]);

              START_TIMING(proc_intersect_ijklmno);
              SET_INTERSECT(intersectionBuffer, scratchBufferijklmn, rowO,
                            dataTable.tumorRowUnits);
              END_TIMING(proc_intersect_ijklmno,
                         elapsed_times[PROCESS_LAMBDA_INTERSECT]);
              ...
              SET_INTERSECT7(intersectionBuffer, rowIN, rowJN, rowKN, rowLN,
                             rowMN, rowNN, rowON, dataTable.normalRowUnits);
              ...
              if (F >= maxF) {
                maxF = F;
                bestCombination = {computed.i, computed.j, k, l, m, n, o};
              }
            }
          }
        }
...
bool process_and_communicate(
    int rank, LAMBDA_TYPE num_Comb, double &localBestMaxF,
    std::array<int, NUMHITS> &localComb, LAMBDA_TYPE &begin, LAMBDA_TYPE &end,
    MPI_Status &status, sets_t dataTable, SET &intersectionBuffer,
    SET &scratchBufferij, SET &scratchBufferijk, SET &scratchBufferijkl,
    SET &scratchBufferijklm, SET &scratchBufferijklmn, double elapsed_times[]) {
  ...
  process_lambda_interval(begin, end, localComb, localBestMaxF, dataTable,
                          ...
                          scratchBufferijklmn, elapsed_times);
...
void distribute_tasks(int rank, int size, const char *outFilename,
  ...
  while (
    ...
    SET_INTERSECT7(intersectionBuffer,
