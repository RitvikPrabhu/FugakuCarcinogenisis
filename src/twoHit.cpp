...
inline void process_lambda_interval(LAMBDA_TYPE startComb, LAMBDA_TYPE endComb,
  ...
  for (LAMBDA_TYPE lambda = startComb; lambda <= endComb; lambda++) {
    ...
    SET_INTERSECT(intersectionBuffer, rowI, rowJ, dataTable.tumorRowUnits);
    ...
    SET_INTERSECT2(intersectionBuffer, rowIN, rowJN, dataTable.normalRowUnits);
    END_TIMING(proc_intersect_normal, elapsed_times[PROCESS_LAMBDA_INTERSECT]);

    START_TIMING(proc_count_coveredNormal);
    int coveredNormal = SET_COUNT(intersectionBuffer, dataTable.normalRowUnits);
    ...
    if (F >= maxF) {
      maxF = F;
      bestCombination = {computed.i, computed.j};
    }
  }
}
...
void distribute_tasks(int rank, int size, const char *outFilename,
  ...
  while (
    ...
    SET_INTERSECT2(intersectionBuffer,
