        ...
        for (int m = l + 1; m < totalGenes - (NUMHITS - 5); m++) {
          ...
          for (int n = m + 1; n < totalGenes - (NUMHITS - 6); n++) {
            ...
            for (int o = n + 1; o < totalGenes - (NUMHITS - 7); o++) {
              ...
              SET_INTERSECT(scratchBufferijklmno, scratchBufferijklmn, rowO,
                            dataTable.tumorRowUnits);
              END_TIMING(proc_intersect_ijklmno,
                         elapsed_times[PROCESS_LAMBDA_INTERSECT]);
              if (SET_IS_EMPTY(scratchBufferijklmno, dataTable.tumorRowUnits)) {
                continue;
              }

              for (int p = o + 1; p < totalGenes - (NUMHITS - 8); p++) {
                START_TIMING(proc_row_p);
                SET rowP =
                    GET_ROW(dataTable.tumorData, p, dataTable.tumorRowUnits);
