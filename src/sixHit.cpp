...
inline void process_lambda_interval(
    ...
    SET &scratchBufferijkl, SET &scratchBufferijklm, double elapsed_times[]) {
  double alpha = 0.1;
      ...
      for (int l = k + 1; l < totalGenes - (NUMHITS - 4); l++) {
        ...
        for (int m = l + 1; m < totalGenes - (NUMHITS - 5); m++) {
          ...
          for (int n = m + 1; n < totalGenes - (NUMHITS - 6); n++) {
            ...
            if (F >= maxF) {
              maxF = F;
              bestCombination = {computed.i, computed.j, k, l, m, n};
            }
          }
        }
      }
...
bool process_and_communicate(int rank, LAMBDA_TYPE num_Comb,
                             ...
                             SET &scratchBufferijklm, double elapsed_times[]) {
