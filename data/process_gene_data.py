...
def process_gene_data(gene_sample_file, normal_gene_list_file, output_file):
    gene_sample_df = pd.read_csv(gene_sample_file, sep="\s+", header=None, 
    ...
    result = gene_sample_pivot.reset_index().melt(id_vars="gene", var_name="sample", value_name="mutation")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python process_gene_data.py <gene_sample_file> <normal_gene_list_file> <output_file>")
        sys.exit(1)
